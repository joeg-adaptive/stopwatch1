...
import { createAndHandleLiveLapTime, $tableRowWithLiveLapTime } from './tableManager.js'
import {
...
export function continouslyRunningTimer($stopWatchClock) {
	...
	$tableRowWithLiveLapTime.innerText = getFormattedTime(Date.now() - startTime - handleAccruedLapTimes('get'))
	theTimer = requestAnimationFrame(() => continouslyRunningTimer($stopWatchClock))
}
...
export function resetValues() {
	...
	document.querySelectorAll('.flexLapRow').forEach((element) => {
